...
int bn_compare_null(bn_node_t n1, bn_node_t n2) {
...
int main() {
  ...
  CHECK_MSG(bn_same_tree(t1, t2, bn_compare_null) == 1, "Hai cây rỗng gốc");
  t1->root = n11;
  CHECK_MSG(bn_same_tree(t1, t2, bn_compare_null) == 0, "Một cây rỗng gốc");
  t2->root = n21;
  CHECK_MSG(bn_same_tree(t1, t2, bn_compare_null) == 1, "Hai cây khác rỗng");
  ...
  CHECK_MSG(bn_same_tree(t1, t2, bn_compare_null) == 1, "Hai cây 1 -> 2 -> 3");
  return 0;
}