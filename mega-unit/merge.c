...
#else  // CGEN_USE_GC
  struct gvector *lines = gvec_create_full(0, 100, gtype_zero);
#endif  // CGEN_USE_GC
  char *tmp = NULL;
...
#else  // CGEN_USE_GC
  struct gvector *contents = gvec_create_full(0, 1000, gtype_zero);
#endif  // CGEN_USE_GC
  char tmp[1024];
  gvec_traverse(cur, v) {
    if (!is_include(cur->s)) {
    ...
    gvec_append(contents, gtype_s(ext_strdup("")));
